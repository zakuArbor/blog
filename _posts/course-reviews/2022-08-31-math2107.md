...
a section dedicated on [discussing the organization of the course](#org) and my overall thoughts about the course.
...
* Attend lectures that are optional for the bonus marks and getting more practice
* A good number of topics overlap from MATH1104 but this course goes more in-depth
...
<tr>                                                                            
  <td>Week 8</td>                                                                
  <td>Dot Products, Norm, Distance, Unit Vectors, Oethogonal sets, orthogonal basis, orthongonal complement, and orthogonal matrices</td>                                                                     
</tr>   
<tr>                                                                            
  <td>Week 9</td>                                                                
...
Oddly enough, MATH2107 begin with working on proofs for a course that does not have an emphasis on proofs. It reminds me how MATH1052 (Calc 1 Honors) and MATH1152 (Linear Algebra I Honors) began with proofs involving 
some field. 
...
which is something I did not see before. A polynomial vector space is simply just polynomials with coefficient in some field such as $\mathbb{R}$ but it was interesting to see a space I did not recognize.
...
Although it is intuitive on how to represent any vector using the standard basis, it's not the only way to compose any vector. 
...
What happens if you want to represent the vector in respect to a different base. So far, we have only been dealing with a single basis but we can introduce 
another basis into the problem. Although not apparent as to why we would do this, it will make more sense when we deal with transformation matrices later in 
...
The next topic covered is eigvenvector and eigenvalues which should look familiar from the previous course. Revisiting this topic 6 years later made me realize how beautiful the expression 
$Av = \lambda v$ is because of the few facts I took forgranted and did not realize till watching Kyle's lectures. The first property is that eigenvalues and eigvenvectors are for <b>SQUARE</b> matrices. 
But that should have been obvious anyways (I was a brainless student back in the days). The neat feature of the equation is that if you know the eignvalue $\lambda$, you can compute the result of Ax 
...
about eigvenvectors is that two eigven vectors belonging to two different eigenvalues. Other topics covered is finding determinants which may also be review. The course covers finding the determinants 
...
This should all be review (in my case I completely forgot I learnt this 
till after I finished the course. It's been about 6 years since I learnt this). Something neat I learned from the course is that:
...
You may or may not be wondering what is the significance of this aside from looking erily similar to the characteristic equation. 
It turns out that to find the eigenvectors for each eigenvalue, it's simply the $nul(A - I \lambda)$ meaning the product of 
$(A - I \lambda)$ with the basis of the eigenbasis you found for that particular eigen value should be equal to 0. This is one 
way to confirm if your answer is correct. This actually applies anytime you find vectors for the nullspace (i.e. by definition 
...
<p class = "caption">A sample mindmap of linear algebra and this is only one of three mindmaps from that minireference.com</p>

On a side note, recall learning about complex numbers in the previous course? Being able to manipulate complex numbers comes very 
...
complex eigenvalues despite the matrix having all their entries being real. Row reducing complex matrices are a huge pain. The easiest 
method to dealing with row reducing complex matrices is to multiply the column you are working with by its conjugate. You will see when you 
...
covered. Probably not required but a neat trick to compute $i^n$ is to split $i^n$ into parts using the exponent laws: $i^n = (i^2)^{n-2} = (-1)^{n-2}$. If n-2 is 
even then the answer is a 1, if it's odd, then there are more thinking required.
...
proof to prove some transformation is linear or not. An interesting property of linear transformation is that if $T(v_1), T(v_2), ..., T(v_n)$ are linearly indepedent 
...
transformation for each basic vector, then you can determine what the output is for any vector in the basis. Perhaps I am over-exaggerating how fascinating this is, 
but the ability to obtain the output of a linear transformation without knowing the transformation is neat. In other words, you are able to know the output of any value 
from a function knowing some output of the basic essential inputs. As you probably can tell, I really like concepts that are able to represent data in different ways and 
...
set of vectors such that $T(\overrightarrow{0}) = \overrightarrow{0}$ and the nullity is the dimension of the kernel. I always found the name kernel confusing. Hopefully one day I'll learn as to why Mathematicians 
call it the kernel because when I think of a kernel, I think of the core of an operating system (i.e. Linux Kernel). Similarly to the nullspace, you can verify your answer by inputting the basis vectors you found 
...
and kernel of a linear transformation is very similar to finding the nullspace and the colspace of a matrix. The similarities between kernel and range with nullspace and colspace is very apparent especially 
...
The course then shifts gears to talk about dot products, norm, unit vectors and orthongoal sets which may or may not be a review. I say everything may or may not be review but to be honest, I only recall their 
...
to know whether or not the vectors are orthogonal which is when the dot product between two vectors are equal to 0. In this course, we don't define the dot product to be $AB\cos\theta$ (recall $\cos 90^\circ = 0$ as 
...
$u \cdot v = u_1 v_1 + u2 v_2 + ... + u_n v_n$). What confused me about learning about norm is that I always though the norm meant the perpendicular line. But in this course, it means the distance of the vector 
from the origin. From this point onwards, the course talks a great deal about orthogonality of vectors and matrices. As stated earlier, two vectors are orthogonal if their dot products are 0 so by extension, an 
orthogonal set is simply the set of vectors that are orthogonal. To check if a set is orthogonal, you just perform the dot product on each vector in set which made me realize an interesting fact. The amount of times 
...
and is the linear combination of any vector). If an orthogonal set is linearly independent then one could find the linear combination of any vector $w$ belonging to the span of the orthogonal set simply using some formula to 
...
unit vectors). This introduces to the idea of orthonormal columns and orthogonal matrices where the property of $U^TU = I$ where U is some matrix that has orthonormal columns (this a way to verify 
if you calculated the orthonormal set correctly). This is important to remember when we work with 
QR factorization and finding the orthogonal projections which we will see later on. The course also introduces to Orthogonal complement $W^\perp$ which may be familiar (I did not realize I learnt this before till I 
found my old notes from my transfer credit for MATH1104 the day after I wrote MATH2107 exam). The orthogonal complement is the set of vectors that are prependicular to everything in some set W. While it may not be apparent 
why or what this means visually, but everything will make sense in the next topic. Orthogonal projection is the idea of "projecting" (drawing) a vector within some space (plane) or within some vector that is closest point 
...
and Zach Star's (you may have remember him as MajorPrep) [application videos](https://www.youtube.com/watch?v=rowWM-MijXU&list=PL7jXONz_Ob4vheCTRDc4jhC_7BJAGvMST&index=2&t=4s) do a great job in making linear algebra 
more intuitive (I really should buy their merchs one day to support their works because it's simply amazing). Anyhow, orthogonal decomposition is simply breaking down the vectors that make up y (i.e. $y = \hat{y} + z$) 
which is basically vector algebra you learned in Highschool. Rearranging the equation gives you the ability to find $z \in W^\perp$, the perpendicular line that intersects $\hat{y}$ and the vector y. A question that is: 
...
If the pythagorean theorem is violated, then you have calculated either $\hat{y}$ or $z$ incorrectly as they will not be in their respective spanning set. The next topic introduced is still about projections but ties in 
...
fact). Another thing you may have notice that since the topics are so interconnected (i.e. a very dense graph if you get what I am saying), there are so many ways to approach a problem. That is where the course can get very 
confusing. The next topic is the Gram-Schmidt Process, a keyword I remember learning before but had no recollection how it works. Gram-Schmidt process is a neat but very annoying process (in my own opinion) to convert 
...
it's their inside joke I keep overhearing during lectures. I remember the equation $R = Q^TA$ as "OTA" (over-the-air) update except instead of an "O", it's a "Q". Eventually I forgot the acrynom after the test that for the 
...
multipled top row incorrectly which was why the check if the matrix is an upper-triangular matrix did not catch the error beforehand. Checking your work by utilizing the definitions and theorems are a good way 
...
![A crude way to find the line of best fit](../assets/math-physics/phys-least-squares-crude.jpg)
...
remaining days of August or when I take MATH2100 - Algebra. Anyhow, a lot of the properties of dot products such as the work for orthogonality applies to inner products as well. There are three inner products 
...
When discussing about symmetric matrices, the idea of a diagonalizable matrix shows up once again. Previously it was stated that a matrix is diagonalizable if the matrix can be represented as $A = PDP^{-1}$ where 
P contains the eigenbasis as columns. However, this time the idea of diagonalization becomes stricter by adding a condition that the eigenbasis must consists of vectors that are orthonormal to each other within the same eigenbasis. 
This stricter definition of diagonalization is unsurprisingly called the orthogonal diagonlization and is written as $A = UDU^T$ which looks oddly similar to the definition of the diagonalizable matrix: $A = PDP^{-1}$. 
The direct consequence of this theorem is that for two vectors $v_1$ and $v_2$ belonging to two different eigenbasis, $v_1$ and 
...
of a symmetric matrix are also real or how A can be represented as a spectral decomposition which I am still trying to wrap my head around why this was introduced. Perhaps its because its part of the Spectral Theorem 
which has nice properties that can be exploited for the next topic about Quadratic Form and the Principle Axis Theorem.

A quadratic form as the name implies is simply an equation of polynomials that have at most the degree of 2 (i.e. $x^2$). Quadratic form is defined to be the function $Q(x) = x^TAx$ where A is a symmetric. An example of a 
quadratic form is $Q(x) = -3x_1^2 + 9x_1x_2 - 2x_2^2$ (taken from someone's blog post since I wanted to stop copying examples from the lecture slides and too lazy to come up with one). 
Notice the term $9x_1x_2$ are not polynomial of degree 2 (or not in second order). This term is referred to as the cross-product term which is not desirable which is why the Principal Axis Theorem was introduced. This 
...
but replacing the variables to be $Q(x) = y^TDy$. The second way I remember this form is remembering that the Principle Axis Theorem relies on substituting x = Uy into $Q(x) = x^TAx$ which results in $Q(x) = x^TAx$ if 
...
I definitely should revisit robotics one day and tackle the subject as my math ability has vastly improved since then (though we shall see if MATH2000 and MATH2100 breaks me inside to not want to touch math 
...
which is either a yay or nay depending who you ask. As a confession, it was a yay for me despite being in the Math honors program who should be taking the honors version 
of this course MATH2152. I was originally supposed to take MATH2152 in the winter but an opportunity arised where I got to work at a company specializing in realtime 
operating system as a support developer (think of it as being the person who answers stackoverflow questions but to customers). Due to this arrangement and knowing how 
...
MATH2107 is formatted such that the course was a "reverse-style" classroom which made me extremely scared of initially due to prior experience with reverse-style 
class. However, Kyle organized the course very well such that the course is very clear and his lecture videos and slides had all the information you needed to 
succeed in the course. The reverse style classes I took involved an extreme amount of self studying and agonizing why your code doesn't work. For context, I took 
an introductory to system programming course that was reverse styled where we had to do a series of excercises that were counted to our marks before lecture. Lectures 
were for working on problems together with the professor and answering questions that students strugged on the excercises that we submitted ... Anyhow, this is different. 
It was not an anxiety nor frustrating experience I had previously but a very fun experience. I loved taking MATH2107 with Kyle Harvey and I would have to say it is the 
...
(to intice students to come to class). 
While probably less than half of the class showed up to lectures, the problems we worked on every lecture was very helpful. Every lecture, 
...
These points accumulate to a maximum of 40 points throughout the course which gives you a bonus of 2.5% towards your final grade which usually results in an increase of a letter grade. 
...
that I would learn better if I do these problems myself. Though it would be nice to have a partner whom you could compare your answers before submitting but I just bothered Kyle instead 
whenever I had a question or to see if I was on the right track. The reason why lectures are spent practicing problems is that students often do bad in Math because they lack practice. 
This course requires a lot of practice, especially if you have been relying on a RREF calculator in the previous course due to previous course being online. Linear Algebra is often a hit 
or miss course from seeing the grades of my friend when I took MATH1104 equivalent many years ago. Some of my friends would do way better in calculus compared to Linear Algebra where 
...
on the other hand ... was not great, but at least I did way better than the class average which was a D+). Though I remember nothing after I wrote that exam in my first Linear Algebra course 
...
One of the greatest thing I observed in the lecture is hearing people's lightbulb turn on when they finally understand 
some concept through the guidance of Kyle. This is what reverse style classroom is supposed to be about or at least in active learning environment. Kyle is experienced at teaching students 
...
a frustrating experience. I've been told that studies has shown that reverse style classroom yields better results. But from my observation of reverse style classrooms I've taken or 
...
There is a tutorial for the course but no one goes from my personal experience as I attended all of them. I would be in tutorial and there would be at most 2 other students. Sometimes a third student 
...
on how I should answer the question or if I got the notation incorrectly. For some reason in the course, I got mixed up with not writing the word "span" or mix up some small nuances in the 
definition so it was helpful to get my work critique. I usually work on the tutorial problems before coming to the tutorials but there were times where I slipped behind and only done half of the 
problems he wanted to cover for that week. Each lesson (there are two or more lessons per week) have a set of questions broken into three parts. The first part checks if you understood the lecture by 
asking you definitions or ask you to describe how you would answer a certain type of question. The second part is practicing the concepts through a series of excercises. The last part of the 
lesson questions (which is titled homework) is the thinking question which is geared towards those who want to do Honors level Math. These problems can be quite interesting so I would try to answer 
the questions and see how far I can go before I give into temptation and read the solutions. One of the greatest thing about proofs is that it helps you understand the concept better by understanding 
how or why a particular theorem, lemma, corollary, or formula works. Sometimes I get curious as to why some theory is true so I would check the proofs that Kyle provides on Brightspace for anyone who is 
curious. Talking about thining questions, Kyle typically puts a thinking question at the end of every bonus lecture problems (Question 6) where students need to think more deeper as it's not obvious and 
is to give students a small taste what honors math is like. I deeply appreciate the fact he tries to introduce bits of theory/thinking questions for those interested in Honors Math because the problems 
he selects are not overwhelming that students cannot figure out (and Kyle was always there to provide hints whenever you get stuck). Prior to taking the course, I did have exposure learning math from Kyle 
...
during the session which forshadows some of the concepts covered in this course. Just a side tangent I wanted to mention.

I do feel somewhat guilty that my desk hogs Kyle the most during lecture and makes me wonder if we are taking too much of his time compared to the people in the back of the class. I do ask a lot of questions 
and so does the group next to me. I don't have any statistical data nor did I make an effort to make a rough estimate on how much time Kyle spends on my group but I would guess it's over a third of the entire 
lecture he would spend with us (I want to say at least half though). In terms of office hours, almost no one attends office hours that Kyle holds which is somewhat surprising (not really since it is summer). 
...
the lecture slides. The tests were very straightforward is all I can say and the same applies for the exam.
...
I submitted the exam. The exam was in-person so it was nice to have MATH2107 be my first in-person exam at CarletonU as I expect all my exams to be in-person from now on after being spoiled with online exams 
where I can reference my cheat sheet that I wrote specifically for the tests and exams in the fall and winter since they were open-book. It is a lie that this was my first in-person exam at CarletonU as 
I took GEOG1020 at CarletonU in the summer 6 years ago when I came over to visit my parents for the summer (I was studying in the GTA at the time). But you get the point.
...
fall semester starts) simply because I enjoyed talking with Kyle and the difficulty of the course wasn't big which allowed me to dig into the course materials without trying to play catch-up and go at on my own pace. 
...
have attempted every proof in which Kyle provided the proof for but I am not that diligent of a student to go through every single proof. I just select which proofs I was interested in, think about it for a few minutes 
and then look at the proof itself. I probably could have handled more challenges than what I encountered in the course as I am a bit more experienced as a student. This course I believe anyone can do well if they put 
in the work as opposed to honors math where I think it's a hit or miss depending on the individual if putting in more work will lead to getting a good mark. I am not saying putting more work in honors math don't give 
you the returns but I think the return is much smaller than a regular math course such as MATH2107. I've seen many students who put in a lot of work in courses (from any department) but their grades were not good considering 
the amount of effort they put in. This made me realize some students learn at different paces (which isn't too surprising) as I often put in a lot of work to only get marginally better grades than others when I 
studied Computer Science (though there are others who put in way more than I did and got worse grades than me but they were my inspiration to take school seriously). Sometimes students need time to familiarize 
the concepts and be exposed to different subjects and ideas before revisiting something they once struggled in. I personally was not good at math (I still am not) but my exposure to math and computer science 
makes me think I can now tackle math (not sure about physics though ...). This was a very big side tangent. 
...
be working in Ottawa during the summer. I apologize for the lack of anime and manga references which I know some students have commented they like (though that's for my course reviews in Computer Science 
where I allowed commenting and have analytics for in my old blog site).
